  ...
  *q++ = 0x00;
  ...
  for(; i < mpegts_muxer->fragment_size_; ++i) {
    switch(mpegts_muxer->fragment_[i].trak->mdia_->hdlr_->handler_type_) {
      case FOURCC('s', 'o', 'u', 'n'): q = write_8(q, 0x0f); break;
      case FOURCC('v', 'i', 'd', 'e'): q = write_8(q, 0x1b); break;
    }
...
unsigned char *get_file_chunk(mp4_context_t *mp4_context, uint64_t pos, size_t size) {
  ...
  mp4_context->file->offset = pos + size;

  if(n == NGX_ERROR) {
    MP4_ERROR("Error reading sample at pos %d", pos);
    return 0;
  }
  return box_data;
}
